...
bool CLZObject::Compress()
{
    ...
    *(DWORD *) pbBuffer = ms_dwFourCC;
    ...
    int r = lzopro_lzo1x_1_16_compress((BYTE *) m_pbIn, m_pHeader->dwRealSize, pbBuffer, (lzo_uint*) &iOutLen, CLZO::Instance().GetWorkMemory());
...
bool CLZObject::Decompress(DWORD * pdwKey)
{
    ...
    if (m_pHeader->dwEncryptSize)
    {
		...
		if (LZO_E_OK != (r = lzopro_lzo1x_decompress_a07(pbDecryptedBuffer + sizeof(DWORD), m_pHeader->dwCompressedSize, m_pbBuffer, (lzo_uint*) &uiSize, nullptr)))
		{
    ...
    else
    {
		...
		if (LZO_E_OK != (r = lzopro_lzo1x_decompress_a07(m_pbIn, m_pHeader->dwCompressedSize, m_pbBuffer, (lzo_uint*) &uiSize, nullptr)))
		{
...
CLZO::CLZO() : m_pWorkMem(nullptr)
{
    ...
    m_pWorkMem = (BYTE *) malloc(LZOPRO_LZO1X_1_16_MEM_COMPRESS);
