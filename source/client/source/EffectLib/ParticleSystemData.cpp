...
void CParticleSystemData::BuildDecorator (CParticleInstance* pInstance)
{
	...
	if (m_ParticleProperty.m_TimeEventAirResistance.size() > 1)
	{
		...
		pInstance->m_pDecorator = pInstance->m_pDecorator->AddChainFront (
									  CAirResistanceValueDecorator::New(m_ParticleProperty.m_TimeEventAirResistance, &pInstance->m_fAirResistance)
								  );
	}
	...
	if (m_ParticleProperty.m_TimeEventGravity.size() > 1)
	{
		...
		pInstance->m_pDecorator = pInstance->m_pDecorator->AddChainFront (
									  CGravityValueDecorator::New(m_ParticleProperty.m_TimeEventGravity, &pInstance->m_fGravity)
								  );
	}
	...
	#ifdef WORLD_EDITOR
	pInstance->m_pDecorator = pInstance->m_pDecorator->AddChainFront (
								  CColorValueDecorator::New(m_ParticleProperty.m_TimeEventColorRed, &pInstance->m_Color.r));
	pInstance->m_pDecorator = pInstance->m_pDecorator->AddChainFront (
								  CColorValueDecorator::New(m_ParticleProperty.m_TimeEventColorGreen, &pInstance->m_Color.g));
	pInstance->m_pDecorator = pInstance->m_pDecorator->AddChainFront (
								  CColorValueDecorator::New(m_ParticleProperty.m_TimeEventColorBlue, &pInstance->m_Color.b));
	pInstance->m_pDecorator = pInstance->m_pDecorator->AddChainFront (
								  CColorValueDecorator::New(m_ParticleProperty.m_TimeEventAlpha, &pInstance->m_Color.a));
	#else
	pInstance->m_pDecorator = pInstance->m_pDecorator->AddChainFront (
								  CColorAllDecorator::New(m_ParticleProperty.m_TimeEventColor, &pInstance->m_dcColor));
	#endif

	pInstance->m_pDecorator = pInstance->m_pDecorator->AddChainFront (
								  CScaleValueDecorator::New(m_ParticleProperty.m_TimeEventScaleX, &pInstance->m_v2Scale.x));
	pInstance->m_pDecorator = pInstance->m_pDecorator->AddChainFront (
								  CScaleValueDecorator::New(m_ParticleProperty.m_TimeEventScaleY, &pInstance->m_v2Scale.y));
	...
	if (m_ParticleProperty.GetTextureAnimationFrameCount() > 1 && m_ParticleProperty.GetTextureAnimationFrameDelay() > 1e-6)
	{
		switch (pInstance->m_byTextureAnimationType)
		{
			case CParticleProperty::TEXTURE_ANIMATION_TYPE_CW:
				pInstance->m_pDecorator = pInstance->m_pDecorator->AddChainFront (
											  CTextureAnimationCWDecorator::New(m_ParticleProperty.GetTextureAnimationFrameDelay(), m_ParticleProperty.GetTextureAnimationFrameCount(), &pInstance->m_byFrameIndex));
				break;
			case CParticleProperty::TEXTURE_ANIMATION_TYPE_CCW:
				pInstance->m_pDecorator = pInstance->m_pDecorator->AddChainFront (
											  CTextureAnimationCCWDecorator::New(m_ParticleProperty.GetTextureAnimationFrameDelay(), m_ParticleProperty.GetTextureAnimationFrameCount(), &pInstance->m_byFrameIndex));
				break;
			case CParticleProperty::TEXTURE_ANIMATION_TYPE_RANDOM_FRAME:
				pInstance->m_pDecorator = pInstance->m_pDecorator->AddChainFront (
											  CTextureAnimationRandomDecorator::New (m_ParticleProperty.GetTextureAnimationFrameDelay(), m_ParticleProperty.GetTextureAnimationFrameCount(), &pInstance->m_byFrameIndex));
				break;
		}
	}
	...
	switch (byRotationType)
	{
		case CParticleProperty::ROTATION_TYPE_TIME_EVENT:
			...
			pInstance->m_pDecorator = pInstance->m_pDecorator->AddChainFront (
										  CRotationSpeedValueDecorator::New (m_ParticleProperty.m_TimeEventRotation, &pInstance->m_fRotationSpeed));
			break;
