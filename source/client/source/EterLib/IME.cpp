...
protected:
	...
	static BOOL SetupCompartmentSinks( BOOL bResetOnly = FALSE, ITfCompartment* pTfOpenMode = nullptr, ITfCompartment* ppTfConvMode = nullptr );
...
void CIME::PasteTextFromClipBoard()
{
	...
	if (!buffer) // Dont pass nullptr to std::string
	{
		GlobalUnlock(handle);
		CloseClipboard();
		return;
