...
short CGraphicTextInstance::GetEmoticonX(int n)
{
	std::vector<SEmoji>::iterator it;
	int j=0;

	for (it = m_emojiVector.begin(); it != m_emojiVector.end(); it++, j++)
		if (j == n)
			return it->x;
...

	{
		...
		if (m_isOutline)
		{
			...
			for (i=m_pCharInfoVector.begin(); i!=m_pCharInfoVector.end(); ++i)
			{
				...
				static BYTE abColor[4];
				memcpy(abColor, &m_dwOutLineColor, sizeof(abColor));
				if (m_isAlpha)
					abColor[3] = m_fAlpha * 255.0f;

				akVertex[3].color = akVertex[2].color = akVertex[1].color = akVertex[0].color = *(DWORD*)abColor;
		...
		for (int i = 0; i < m_pCharInfoVector.size(); ++i)
		{
			...
			//m_dwTextColor;
			static BYTE abColor[4];
			memcpy(abColor, &m_dwColorInfoVector[i], sizeof(abColor));
			if (m_isAlpha)
				abColor[3] = m_fAlpha * 255.0f;

			akVertex[0].color = akVertex[1].color = akVertex[2].color = akVertex[3].color = *(DWORD*)abColor;
...
{
	D3DXCOLOR* pD3DXColor = &((D3DXCOLOR)color);
...
void CGraphicTextInstance::SetAlpha(float fAlpha)
{
	m_isAlpha = true;
	m_fAlpha = fAlpha;
}

float CGraphicTextInstance::GetAlpha() const
{
	return m_fAlpha;
}
