...
bool CAttributeData::OnLoad(int iSize, const void * c_pvBuf)
{
	...
	if (c_iAttributeDataFileHeaderLength + 1 > iSize)
	{
		TraceError("invalid size of attribute resource file [size need %d size got %d]", c_iAttributeDataFileHeaderLength + 1, iSize);
		return false;
	}
	...
	iSize -= c_iAttributeDataFileHeaderLength+1;
	if (strcmp(szHeader, c_szAttributeDataFileHeader))
		return FALSE;

	if (sizeof(DWORD) * 2 > iSize)
	{
		TraceError("invalid size of attribute resource file 2 [size need %d size got %d]", sizeof(DWORD) * 2, iSize);
		return false;
	}
	...
	{
		if (sizeof(DWORD) + 32 + sizeof(D3DXVECTOR3) > iSize)
		{
			TraceError("invalid size of attribute resource file 3 [size need %d size got %d]", sizeof(DWORD) + 32 + sizeof(D3DXVECTOR3), iSize);
			return false;
		}
		...
		iSize -= sizeof(D3DXVECTOR3);

		switch(rCollisionData.dwType)
		{
			case COLLISION_TYPE_PLANE:
				if (sizeof(float)*2 > iSize)
				{
					TraceError("invalid size of attribute resource file 4 [size need %d size got %d]", sizeof(float)*2, iSize);
					return false;
				}
			...
			case COLLISION_TYPE_BOX:
				if (sizeof(float)*3 > iSize)
				{
					TraceError("invalid size of attribute resource file 5 [size need %d size got %d]", sizeof(float)*3, iSize);
					return false;
				}
			...
			case COLLISION_TYPE_SPHERE:
				if (sizeof(float) > iSize)
				{
					TraceError("invalid size of attribute resource file 6 [size need %d size got %d]", sizeof(float), iSize);
					return false;
				}
			...
			case COLLISION_TYPE_CYLINDER:
				if (sizeof(float)*2 > iSize)
				{
					TraceError("invalid size of attribute resource file 7 [size need %d size got %d]", sizeof(float)*2, iSize);
					return false;
				}
				memcpy(rCollisionData.fDimensions, c_pbBuf, 2*sizeof(float));
				c_pbBuf += 2*sizeof(float);
			...
			case COLLISION_TYPE_AABB:
				if (sizeof(float)*3 > iSize)
				{
					TraceError("invalid size of attribute resource file 8 [size need %d size got %d]", sizeof(float)*3, iSize);
					return false;
				}
				memcpy(rCollisionData.fDimensions, c_pbBuf, 3*sizeof(float));
				c_pbBuf += 3*sizeof(float);
			...
			case COLLISION_TYPE_OBB:
				if (sizeof(float)*3 > iSize)
				{
					TraceError("invalid size of attribute resource file 9 [size need %d size got %d]", sizeof(float)*3, iSize);
					return false;
				}
				memcpy(rCollisionData.fDimensions, c_pbBuf, 3*sizeof(float));
				c_pbBuf += 3*sizeof(float);
		...
		if (sizeof(D3DXQUATERNION) > iSize)
		{
			TraceError("invalid size of attribute resource file 10 [size need %d size got %d]", sizeof(D3DXQUATERNION), iSize);
			return false;
		}
		memcpy(rCollisionData.quatRotation, c_pbBuf, sizeof(D3DXQUATERNION));
		c_pbBuf += sizeof(D3DXQUATERNION);
		iSize -= sizeof(D3DXQUATERNION);
	}

	for (DWORD j = 0; j < dwHeightDataCount; ++j)
	{
		if (32 + sizeof(DWORD) > iSize)
		{
			TraceError("invalid size of attribute resource file 11 [size need %d size got %d]", 32 + sizeof(DWORD), iSize);
			return false;
		}
		...
		if (dwPrimitiveCount*sizeof(D3DXVECTOR3) > iSize)
		{
			TraceError("invalid size of attribute resource file 12 [size need %d size got %d]", dwPrimitiveCount*sizeof(D3DXVECTOR3), iSize);
			return false;
		}
		...
		iSize -= dwPrimitiveCount*sizeof(D3DXVECTOR3);
