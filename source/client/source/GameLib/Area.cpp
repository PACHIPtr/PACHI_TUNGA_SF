...
void CArea::__Load_BuildObjectInstances()
{
	...
	{
		*it = ms_ObjectInstancePool.Construct();
		(*it)->Clear();
...
{
	pObjectInstance->pAmbienceInstance = ms_AmbienceInstancePool.Construct();
...
void CArea::__SetObjectInstance_SetDungeonBlock(TObjectInstance * pObjectInstance, const TObjectData * c_pData, CProperty * pProperty)
{
	...
	pObjectInstance->pDungeonBlock = ms_DungeonBlockInstancePool.Construct();
...
void CArea::__LoadAttribute(TObjectInstance * pObjectInstance, const char * c_szAttributeFileName)
{
	...
	CAttributeInstance * pAttrInstance = ms_AttributeInstancePool.Construct();
	pAttrInstance->Clear();
	...
	{
		pAttrInstance->Clear();
		ms_AttributeInstancePool.Destruct(pAttrInstance);
	}
}
...
bool CArea::__Load_LoadObject(const char * c_szFileName)
{
	...
	for (DWORD i = 0; i < dwCount; ++i)
	{
		...
		if (!CPropertyManager::Instance().Get(ObjectData.dwCRC, &pProperty))
		{
			...
			TraceError(" xxxxxt Property(%s) Load ERROR", c_szFileName);
...
void CArea::__Clear_DestroyObjectInstance(TObjectInstance * pObjectInstance)
{
	...
	if (pObjectInstance->pAttributeInstance)
	{
		...
		ms_AttributeInstancePool.Destruct(pObjectInstance->pAttributeInstance);
	...
	{
		ms_AmbienceInstancePool.Destruct(pObjectInstance->pAmbienceInstance);
		pObjectInstance->pAmbienceInstance = nullptr;
	}

	if (pObjectInstance->pDungeonBlock)
	{
		ms_DungeonBlockInstancePool.Destruct(pObjectInstance->pDungeonBlock);
		pObjectInstance->pDungeonBlock = nullptr;
	}

	pObjectInstance->Clear();
	
	ms_ObjectInstancePool.Destruct(pObjectInstance);
}
