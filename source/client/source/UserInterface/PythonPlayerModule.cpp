...
#endif
PyObject * playerPickCloseAutoItem(PyObject* poSelf, PyObject* poArgs)
{
	int iValue;
	if (!PyTuple_GetInteger(poArgs, 0, &iValue))
		return Py_BuildException();
	
	CPythonPlayer::Instance().PickCloseAutoItem(iValue);
	return Py_BuildNone();
}
...
{
	CInstanceBase * pInstance = CPythonPlayer::Instance().GetMainActorPtr();
	if (!pInstance)
...
PyObject * playerGetYearRoundPoint(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("i", CPythonPlayer::Instance().GetStatus(POINT_YEAR_ROUND));
}
#endif
...
PyObject * playerGetItemCountByVnumNew(PyObject* poSelf, PyObject* poArgs)
{
	...
	int ItemNum = CPythonPlayer::Instance().GetItemCountByVnum(ivnum);
	for (int i = 0; i < c_Skill_Book_Inventory_Slot_Count; ++i)
	{
		if (ivnum == CPythonPlayer::Instance().GetItemIndex(TItemPos(SKILL_BOOK_INVENTORY, i)))
		{
			ItemNum += CPythonPlayer::Instance().GetItemCount(TItemPos(SKILL_BOOK_INVENTORY, i));
		}
	}
	for (int i = 0; i < c_Upgrade_Items_Inventory_Slot_Count; ++i)
	{
		if (ivnum == CPythonPlayer::Instance().GetItemIndex(TItemPos(UPGRADE_ITEMS_INVENTORY, i)))
		{
			ItemNum += CPythonPlayer::Instance().GetItemCount(TItemPos(UPGRADE_ITEMS_INVENTORY, i));
		}
	}
	for (int i = 0; i < c_Stone_Items_Inventory_Slot_Count; ++i)
	{
		if (ivnum == CPythonPlayer::Instance().GetItemIndex(TItemPos(STONE_ITEMS_INVENTORY, i)))
		{
			ItemNum += CPythonPlayer::Instance().GetItemCount(TItemPos(STONE_ITEMS_INVENTORY, i));
		}
	}
	for (int i = 0; i < c_Chest_Items_Inventory_Slot_Count; ++i)
	{
		if (ivnum == CPythonPlayer::Instance().GetItemIndex(TItemPos(CHEST_ITEMS_INVENTORY, i)))
		{
			ItemNum += CPythonPlayer::Instance().GetItemCount(TItemPos(CHEST_ITEMS_INVENTORY, i));
		}
	}
	for (int i = 0; i < c_Attr_Items_Inventory_Slot_Count; ++i)
	{
		if (ivnum == CPythonPlayer::Instance().GetItemIndex(TItemPos(ATTR_ITEMS_INVENTORY, i)))
		{
			ItemNum += CPythonPlayer::Instance().GetItemCount(TItemPos(ATTR_ITEMS_INVENTORY, i));
		}
	}
	for (int i = 0; i < c_Flower_Items_Inventory_Slot_Count; ++i)
	{
		if (ivnum == CPythonPlayer::Instance().GetItemIndex(TItemPos(FLOWER_ITEMS_INVENTORY, i)))
		{
			ItemNum += CPythonPlayer::Instance().GetItemCount(TItemPos(FLOWER_ITEMS_INVENTORY, i));
		}
	}
	return Py_BuildValue("i", ItemNum);
}
...
PyObject * playerGetItemLink(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (pPlayerItem && CItemManager::Instance().GetItemDataPointer(pPlayerItem->vnum, &pItemData))
	{
		...
		len = snprintf(itemlink, sizeof(itemlink), "item:%x:%x:%x:%x:%x:%x:%d:%d",
			...
			pPlayerItem->alSockets[0], pPlayerItem->alSockets[1], pPlayerItem->alSockets[2], pPlayerItem->alSockets[3], pPlayerItem->transmutation, pPlayerItem->count);
...
PyObject * playerCanRefine(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (pScrollItemData->GetValue(0) == 11 && pTargetItemData->GetLevelLimit() < pScrollItemData->GetValue(1))
	{
		return Py_BuildValue("i", REFINE_CANT);
	}
	
	if (pTargetItemData->GetType() == CItemData::ITEM_TYPE_SOUL)
...
{
	CPythonNetworkStream::Instance().SendExtendInvenUpgrade();
	return Py_BuildNone();
}
...
PyObject * playerSendExtendInvenButtonClick(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream::Instance().SendExtendInvenButtonClick(index);
	return Py_BuildNone();
}
#endif
...
PyObject * playerGetZodiacPoint(PyObject * poSelf, PyObject* poArgs)
{
	return Py_BuildValue("i", CPythonPlayer::Instance().GetStatus(POINT_ZODIAC));
}
#endif

#ifdef ENABLE_WORSHIP_SYSTEM
PyObject * playerGetWorshipPoint(PyObject * poSelf, PyObject * poArgs)
{
	return Py_BuildValue("i", CPythonPlayer::Instance().GetStatus(POINT_WORSHIP));
}
#endif
...
PyObject * playerGetGemShopItemVnum(PyObject* poSelf, PyObject* poArgs)
{
	...
	int gemShopType;
	if (!PyTuple_GetInteger(poArgs, 1, &gemShopType))
		return Py_BuildException();
#endif

	const TGemShopItem * c_pItemData;
#ifdef ENABLE_BOSS_GEM_SYSTEM
	if (CPythonPlayer::Instance().GetGemShopItemData(nPos, &c_pItemData, gemShopType))
#else
...
#ifdef ENABLE_BOSS_GEM_SYSTEM
PyObject * playerGetGemBossShopRefreshTime(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("i", CPythonPlayer::Instance().GetGemBossShopRefreshTime());
}
#endif
#endif
...
PyObject * playerGetSkillBookReadCount(PyObject* poSelf, PyObject* poArgs)
{
	int skillidx;
	if (!PyTuple_GetInteger(poArgs, 0, &skillidx))
		return Py_BuildException();

	return Py_BuildValue("i", CPythonPlayer::Instance().GetSkillBookReadCount(skillidx));
}

PyObject * playerGetSkillBookLeftCount(PyObject* poSelf, PyObject* poArgs)
{
	int skillidx;
	if (!PyTuple_GetInteger(poArgs, 0, &skillidx))
		return Py_BuildException();

	return Py_BuildValue("i", CPythonPlayer::Instance().GetSkillBookLeftCount(skillidx));
}
#endif
...
{
	bool bSafePosition;
	if (!PyTuple_GetBoolean(poArgs, 0, &bSafePosition))
		return Py_BuildException();
	
	CPythonPlayer::Instance().UpdateAutoHunt(bSafePosition);
	return Py_BuildNone();
}
#endif
...
void initPlayer()
{
...
#endif
		{ "PickCloseAutoItem",			playerPickCloseAutoItem,			METH_VARARGS },
...
#ifdef ENABLE_EVENT_INFORMATION_SYSTEM
		{ "GetYearRoundPoint", 	playerGetYearRoundPoint, 										METH_VARARGS },
#endif
...
#ifdef ENABLE_ZODIAC_TEMPLE_SYSTEM
		{"GetZodiacPoint", playerGetZodiacPoint, METH_VARARGS},
#endif
#ifdef ENABLE_WORSHIP_SYSTEM
		{"GetWorshipPoint",	playerGetWorshipPoint,	METH_VARARGS},
#endif
	...
	#ifdef ENABLE_BOSS_GEM_SYSTEM
		{ "GetGemBossShopRefreshTime",	playerGetGemBossShopRefreshTime,	METH_VARARGS },
	#endif
#endif
...
#ifdef ENABLE_SKILL_RENEWAL
		{ "GetSkillBookReadCount",			playerGetSkillBookReadCount,					METH_VARARGS },
		{ "GetSkillBookLeftCount",			playerGetSkillBookLeftCount,					METH_VARARGS },
#endif
	...
	PyModule_AddIntConstant(poModule, "STONE_INVENTORY_SLOT_COUNT",			c_Stone_Items_Inventory_Slot_Count);
	PyModule_AddIntConstant(poModule, "CHEST_INVENTORY_SLOT_COUNT",			c_Chest_Items_Inventory_Slot_Count);
	PyModule_AddIntConstant(poModule, "ATTR_INVENTORY_SLOT_COUNT",			c_Attr_Items_Inventory_Slot_Count);
	PyModule_AddIntConstant(poModule, "FLOWER_INVENTORY_SLOT_COUNT",		c_Flower_Items_Inventory_Slot_Count);
#endif
...
#ifdef ENABLE_SPECIAL_INVENTORY_SYSTEM
	PyModule_AddIntConstant(poModule, "SKILL_BOOK_INVENTORY",				SKILL_BOOK_INVENTORY);
	PyModule_AddIntConstant(poModule, "UPGRADE_ITEMS_INVENTORY",			UPGRADE_ITEMS_INVENTORY);
	PyModule_AddIntConstant(poModule, "STONE_ITEMS_INVENTORY",				STONE_ITEMS_INVENTORY);
	PyModule_AddIntConstant(poModule, "CHEST_ITEMS_INVENTORY",				CHEST_ITEMS_INVENTORY);
	PyModule_AddIntConstant(poModule, "ATTR_ITEMS_INVENTORY",				ATTR_ITEMS_INVENTORY);
	PyModule_AddIntConstant(poModule, "FLOWER_ITEMS_INVENTORY",				FLOWER_ITEMS_INVENTORY);
#endif
	...
	PyModule_AddIntConstant(poModule, "BLOCK_EQUIPMENT_REQUEST", BLOCK_EQUIPMENT_REQUEST);
#endif
	// Party
	...
	PyModule_AddIntConstant(poModule, "ON_TOP_WND_BOOK_COMBINATION",		ON_TOP_WND_BOOK_COMBINATION);
	PyModule_AddIntConstant(poModule, "ON_TOP_WND_COSTUME_COMBINATION",		ON_TOP_WND_COSTUME_COMBINATION);
	...
	PyModule_AddIntConstant(poModule, "ON_TOP_WND_AURA",					ON_TOP_WND_AURA);
	PyModule_AddIntConstant(poModule, "ON_TOP_WND_AURA_REFINE",				ON_TOP_WND_AURA_REFINE);
#endif
...
#ifdef ENABLE_EXTEND_INVENTORY_SYSTEM
	PyModule_AddIntConstant(poModule, "EX_INVENTORY_PAGE_COUNT", 1);
