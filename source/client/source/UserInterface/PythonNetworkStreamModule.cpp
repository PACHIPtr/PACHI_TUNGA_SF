...
PyObject* netSendSelectItemPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rns=CPythonNetworkStream::Instance();
	rns.SendSelectItemPacket(TItemPos(bWindowType, iItemPos));
...
PyObject* netSendExtendInvenButtonClick(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendExtendInvenButtonClick(index);
...
PyObject * netSendYoutuberPanel(PyObject* poSelf, PyObject* poArgs)
{
	char * islem;
	if (!PyTuple_GetString(poArgs, 0, &islem))
		return Py_BuildException();
	char * baslik;
	if (!PyTuple_GetString(poArgs, 1, &baslik))
		return Py_BuildException();
	char * link;
	if (!PyTuple_GetString(poArgs, 2, &link))
		return Py_BuildException();
	
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendYoutuberPanel(islem, baslik, link);
	return Py_BuildNone();	
}		

PyObject * netSendYoutuberVideo(PyObject* poSelf, PyObject* poArgs)
{
	char * islem;
	if (!PyTuple_GetString(poArgs, 0, &islem))
		return Py_BuildException();
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendYoutuberVideo(islem, baslik);
	return Py_BuildNone();	
}
#endif
...
{
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendMailBoxClose();
	return Py_BuildNone();
}

PyObject* netSendMailBoxMail(PyObject* poSelf, PyObject* poArgs)
{
	char * nombre_pj;
	if (!PyTuple_GetString(poArgs, 0, &nombre_pj))
		return Py_BuildException();

	char * asunto;
	if (!PyTuple_GetString(poArgs, 1, &asunto))
		return Py_BuildException();

	char * descrip;
	if (!PyTuple_GetString(poArgs, 2, &descrip))
		return Py_BuildException();

	int slot_item;
	if (!PyTuple_GetInteger(poArgs, 3, &slot_item))
		return Py_BuildException();

	int yang;
	if (!PyTuple_GetInteger(poArgs, 4, &yang))
		return Py_BuildException();

#ifdef USE_WON_SYSTEM_MAILBOX
	int won;
	if (!PyTuple_GetInteger(poArgs, 5, &won))
		return Py_BuildException();
#endif
	
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
#ifdef USE_WON_SYSTEM_MAILBOX
	rkNetStream.SendMailBoxMail(nombre_pj, asunto, descrip, slot_item, yang, won);
#else
	rkNetStream.SendMailBoxMail(nombre_pj, asunto, descrip, slot_item, yang);
#endif

	return Py_BuildNone();
}

PyObject* netSendMailBoxNameCheck(PyObject* poSelf, PyObject* poArgs)
{
	char * nombre_pj;
	if (!PyTuple_GetString(poArgs, 0, &nombre_pj))
		return Py_BuildException();
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendMailBoxNameCheck(nombre_pj);

	return Py_BuildNone();
}

PyObject* netSendMailBoxAcceptMail(PyObject* poSelf, PyObject* poArgs)
{

	int id_mail;
	if (!PyTuple_GetInteger(poArgs, 0, &id_mail))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendMailBoxAcceptMail(id_mail);

	return Py_BuildNone();
}

PyObject* netSendMailBoxDeleteMail(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendMailBoxDeleteMail(id_mail);

	return Py_BuildNone();
}

PyObject* netSendMailBoxAcceptAllMail(PyObject* poSelf, PyObject* poArgs)
{

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendMailBoxAcceptAllMail();

	return Py_BuildNone();
}

PyObject* netSendMailBoxDeleteAllMail(PyObject* poSelf, PyObject* poArgs)
{

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendMailBoxDeleteAllMail();

	return Py_BuildNone();
}


PyObject* netSendMailBoxViewMail(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendMailBoxViewMail(id_mail);
...
PyObject* netSendSafeboxSaveMoneyPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rns=CPythonNetworkStream::Instance();
	rns.SendSafeBoxMoneyPacket(SAFEBOX_MONEY_STATE_SAVE, llMoney);
	return Py_BuildNone();
}
...
PyObject* netSendSafeboxWithdrawMoneyPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rns=CPythonNetworkStream::Instance();
	rns.SendSafeBoxMoneyPacket(SAFEBOX_MONEY_STATE_WITHDRAW, llMoney);
	return Py_BuildNone();
}

PyObject* netSendSafeboxSaveChequePacket(PyObject* poSelf, PyObject* poArgs)
{
	WORD wCheque;
	if (!PyTuple_GetInteger(poArgs, 0, &wCheque))
		return Py_BuildException();

	CPythonNetworkStream& rns=CPythonNetworkStream::Instance();
	rns.SendSafeBoxChequePacket(SAFEBOX_CHEQUE_STATE_SAVE, wCheque);
	return Py_BuildNone();
}

PyObject* netSendSafeboxWithdrawChequePacket(PyObject* poSelf, PyObject* poArgs)
{
	WORD wCheque;
	if (!PyTuple_GetInteger(poArgs, 0, &wCheque))
		return Py_BuildException();
	...
	CPythonNetworkStream& rns=CPythonNetworkStream::Instance();
	rns.SendSafeBoxChequePacket(SAFEBOX_CHEQUE_STATE_WITHDRAW, wCheque);
	return Py_BuildNone();
}
#endif

#ifdef ENABLE_AVERAGE_ITEM_PRICE
PyObject* netSendAveragePriceSearch(PyObject* poSelf, PyObject* poArgs)
{
	int vnum;
	...
	CPythonNetworkStream& rns=CPythonNetworkStream::Instance();
	rns.SendAveragePriceSearch(vnum);
	return Py_BuildNone();
}
#endif
...
void initnet()
{
	static PyMethodDef s_methods[] =
	{
...
#ifdef ENABLE_YOUTUBER_SYSTEM
		{ "SendYoutuberPanelPacket",						netSendYoutuberPanel,						METH_VARARGS },
		{ "SendYoutubeVideoPacket",						netSendYoutuberVideo,						METH_VARARGS },
#endif
...
#ifdef ENABLE_MAIL_BOX_SYSTEM
		{ "MailBoxClose",							netSendMailBoxClose,						METH_VARARGS },
		{ "MailBoxMail",							netSendMailBoxMail,							METH_VARARGS },
		{ "MailBoxNameCheck",						netSendMailBoxNameCheck,					METH_VARARGS },
		{ "MailBoxAcceptMail",						netSendMailBoxAcceptMail,					METH_VARARGS },
		{ "MailBoxDeleteMail",						netSendMailBoxDeleteMail,					METH_VARARGS },
		{ "MailBoxAcceptAllMail",					netSendMailBoxAcceptAllMail,				METH_VARARGS },
		{ "MailBoxDeleteAllMail",					netSendMailBoxDeleteAllMail,				METH_VARARGS },
		{ "MailBoxViewMail",						netSendMailBoxViewMail,						METH_VARARGS },
#endif
		...
		{"SendSafeboxSaveChequePacket", netSendSafeboxSaveChequePacket, METH_VARARGS},
		{"SendSafeboxWithdrawChequePacket", netSendSafeboxWithdrawChequePacket, METH_VARARGS},
#endif

#ifdef ENABLE_AVERAGE_ITEM_PRICE
		{"SendAveragePriceSearch", netSendAveragePriceSearch, METH_VARARGS},
#endif
		{ NULL, NULL, NULL },
	};
