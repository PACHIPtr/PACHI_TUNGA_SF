...
#define DEFAULT_VALUE_ALWAYS_SHOW_NAME		2
...
bool CPythonSystem::GetShowOfflineShopFlag()
{
...
{
	m_Config.iShowOfflineShop = iFlag == 1 ? true : false;
}
...
bool CPythonSystem::IsShowDiceInfo()
{
	return m_Config.bShowDiceInfo;
}

void CPythonSystem::SetShowDiceInfo(int iFlag)
{
	m_Config.bShowDiceInfo = iFlag == 1 ? true : false;
}

#ifdef ENABLE_SHOW_NIGHT_SYSTEM
bool CPythonSystem::GetNightMode()
{
	return m_Config.bNightMode;
}

void CPythonSystem::SetNightMode(int iFlag)
{
	m_Config.bNightMode = iFlag == 1 ? true : false;
}
#endif
...
bool CPythonSystem::IsDesertTexturesMode()
{
	return m_Config.bDesertTexturesMode;
}

void CPythonSystem::SetDesertTexturesMode(int iFlag)
{
	m_Config.bDesertTexturesMode = iFlag == 1 ? true : false;
}

bool CPythonSystem::IsEnableSnowFall()
{
	return m_Config.bEnableSnow;
}

void CPythonSystem::SetEnableSnowFall(int iFlag)
{
...
bool CPythonSystem::LoadConfig()
{
	...
	while (fgets(buf, 256, fp))
	{
		...
		else if (!stricmp(command, "WINDOWED"))
		{
			...
			m_Config.iShowOfflineShop = atoi(value) == 1 ? true : false;
		...
		else if (!stricmp(command, "HIDE_SHAMANS"))
			m_Config.bHideShamans = atoi(value) == 1 ? true : false;
#if defined(WJ_SHOW_MOB_INFO)
		...
		else if (!stricmp(command, "SHOW_DICEINFO"))
			m_Config.bShowDiceInfo = atoi(value) == 1 ? true : false;
#ifdef ENABLE_SHOW_NIGHT_SYSTEM
...
#endif
		else if (!stricmp(command, "SNOW_TEXTURES_MODE"))
			m_Config.bSnowTexturesMode = atoi(value) == 1 ? true : false;
		else if (!stricmp(command, "DESERT_TEXTURES_MODE"))
			m_Config.bDesertTexturesMode = atoi(value) == 1 ? true : false;
		else if (!stricmp(command, "SNOW_MODE_ON"))
			m_Config.bEnableSnow = atoi(value) == 1 ? true : false;
#if defined(ENABLE_GRAPHIC_MASK)
