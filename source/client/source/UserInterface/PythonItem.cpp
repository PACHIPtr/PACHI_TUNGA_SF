...
void CPythonItem::Update(const POINT& c_rkPtMouse)
{
	...
	{
		if (itor->second->dwVirtualNumber == 4294967294)
			continue;
		itor->second->Update();
	}
...
void CPythonItem::CreateItem(DWORD dwVirtualID, DWORD dwVirtualNumber, long socket0, long socket1, long socket2, DWORD count, float x, float y, float z, bool bDrop)
{
	...
	TGroundItemInstance *	pGroundItemInstance = m_GroundItemInstancePool.Construct();	
...
void CPythonItem::DeleteAllItems()
{
	...
	for (i= m_GroundItemInstanceMap.begin(); i!=m_GroundItemInstanceMap.end(); ++i)
	{
		...
		m_GroundItemInstancePool.Destruct(pGroundItemInst);
	}
...
void CPythonItem::DeleteItem(DWORD dwVirtualID)
{
	...
	m_GroundItemInstancePool.Destruct(pGroundItemInstance);
...
bool CPythonItem::GetCloseMoney(const TPixelPosition & c_rPixelPosition, DWORD * pdwItemID, DWORD dwDistance)
{
	...
	for (i = m_GroundItemInstanceMap.begin(); i != m_GroundItemInstanceMap.end(); ++i)
	{
		...
		if (i->second->dwVirtualNumber == 4294967294)
			continue;
...
bool CPythonItem::GetCloseItem(const std::string & myName, const TPixelPosition & c_rPixelPosition, DWORD * pdwItemID, DWORD dwDistance)
{
	...
	DWORD dwPriorityCloseItemID = 0;
	DWORD dwPriorityCloseItemDistance = 1000 * 1000;
	...
	for (i = m_GroundItemInstanceMap.begin(); i != m_GroundItemInstanceMap.end(); ++i)
	{
		...
		DWORD dwxDistance = DWORD(c_rPixelPosition.x-pInstance->v3EndPosition.x);
		DWORD dwyDistance = DWORD(c_rPixelPosition.y-(-pInstance->v3EndPosition.y));
		dwDistance = DWORD(dwxDistance*dwxDistance + dwyDistance*dwyDistance);
		...
		if ((pInstance->stOwnership == "" || pInstance->stOwnership == myName) && dwxDistance*dwxDistance + dwyDistance*dwyDistance < dwPriorityCloseItemDistance)
		{
