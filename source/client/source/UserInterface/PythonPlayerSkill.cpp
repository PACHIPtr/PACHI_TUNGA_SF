...
//		return false;
	...
	// NOTE : 들고 있는 무기 체크
	Tracef("Weapon type is %d", pkInstMain->GetWeaponType());
...
#ifdef ENABLE_CURE_SKILL_FIX
		if (rkSkillInst.dwIndex == 109)
			dwMotionIndex -= 2;
#endif	
...
bool CPythonPlayer::__HasItem(DWORD dwItemID)
{
	...
	{
		if (dwItemID == GetItemIndex(TItemPos (SKILL_BOOK_INVENTORY, i)))
			return true;
	}
	for (int i = 0; i < c_Upgrade_Items_Inventory_Slot_Count; ++i)
	{
		if (dwItemID == GetItemIndex(TItemPos (UPGRADE_ITEMS_INVENTORY, i)))
			return true;
	}
	for (int i = 0; i < c_Stone_Items_Inventory_Slot_Count; ++i)
	{
		if (dwItemID == GetItemIndex(TItemPos (STONE_ITEMS_INVENTORY, i)))
			return true;
	}
	for (int i = 0; i < c_Chest_Items_Inventory_Slot_Count; ++i)
	{
		if (dwItemID == GetItemIndex(TItemPos (CHEST_ITEMS_INVENTORY, i)))
			return true;
	}
	for (int i = 0; i < c_Attr_Items_Inventory_Slot_Count; ++i)
	{
		if (dwItemID == GetItemIndex(TItemPos (ATTR_ITEMS_INVENTORY, i)))
			return true;
	}
	for (int i = 0; i < c_Flower_Items_Inventory_Slot_Count; ++i)
	{
		if (dwItemID == GetItemIndex(TItemPos (FLOWER_ITEMS_INVENTORY, i)))
			return true;
	}
#endif	
