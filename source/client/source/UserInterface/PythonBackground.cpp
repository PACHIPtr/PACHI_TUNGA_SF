...
void CPythonBackground::Update(float fCenterX, float fCenterY, float fCenterZ)
{
	...
	if (rkMap.IsEnablePortal())
	{
		...
		{
			ClearPortal();
			auto treeConstIterator = kGetPortalID.m_kSet_iPortalID.begin();
			for (; treeConstIterator != kGetPortalID.m_kSet_iPortalID.end(); ++treeConstIterator)
			{
				AddShowingPortalID (*treeConstIterator);
			}
			RefreshPortal();
