								   ...
								   float node_size)
{
	...
	SpherePack* nearest1 = 0;
	float neardist1 = 1e38f;

	SpherePack* nearest2 = 0;
...
{
	mCallback = callback;
	mRoot->Visibility (f, this, VS_PARTIAL);
}

void SpherePack::Visibility (const Frustum& f, SpherePackCallback* callback, ViewState state)
{
	if (state == VS_PARTIAL)
	{
	...
	if (HasSpherePackFlag (SPF_SUPERSPHERE))
	{
		...
		while (pack)
		{
			pack->Visibility (f, callback, state);
			pack = pack->_GetNextSibling();
		}
...
void SpherePackFactory::RayTraceCallback (const Vector3d& p1,
		const Vector3d& dir,
		float distance,
		const Vector3d&,
		SpherePack* sphere)
...
void SpherePackFactory::VisibilityCallback (const Frustum& f, SpherePack* sphere, ViewState state)
{
	...
	if (link)
	{
		link->Visibility (f, mCallback, state);
	}
}
