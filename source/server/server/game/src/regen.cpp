...
static bool read_line_zodiac(FILE *fp, LPREGEN regen)
{
	...
	while (get_word(fp, szTmp))
	{
		...
		switch (mode)
		{
			...
			case MODE_LEVEL:
				str_to_number(regen->level, szTmp);
				++mode;
				return true;
		}
	}
...
EVENTFUNC(dungeon_regen_event)
{
	...
	if (info == nullptr)
	{
		sys_err("dungeon_regen_event> <Factor> nullptr pointer");
		return 0;
	}
...
static void regen_spawn_zodiac(LPREGEN regen, LPZODIAC pZodiac, bool bOnce)
{
	...
	for (i = 0; i < num; ++i)
	{
		...
		if (regen->type == REGEN_TYPE_MOB)
		{
			ch = CHARACTER_MANAGER::Instance().SpawnMobRange(regen->vnum, regen->lMapIndex, regen->sx, regen->sy, regen->ex, regen->ey, true, false, false, regen->level);
		...
		{
			if (CHARACTER_MANAGER::Instance().SpawnGroupZodiac(regen->vnum, regen->lMapIndex, regen->sx, regen->sy, regen->ex, regen->ey, bOnce ? nullptr : regen, regen->is_aggressive, pZodiac, regen->level))
				++regen->count;
		}
		else if (regen->type == REGEN_TYPE_GROUP_GROUP)
		{
			if (CHARACTER_MANAGER::Instance().SpawnGroupGroupZodiac(regen->vnum, regen->lMapIndex, regen->sx, regen->sy, regen->ex, regen->ey, bOnce ? nullptr : regen, regen->is_aggressive, pZodiac, regen->level))
				++regen->count;
		}
...
EVENTFUNC(zodiac_regen_event)
{
	...
	if (info == nullptr)
	{
		sys_err("zodiac_regen_event> <Factor> nullptr pointer");
		return 0;
	}
	...
	while (true)
	{
			...
			tmp.type == REGEN_TYPE_ANYWHERE)
		{
			...
			if (!bOnce && pZodiac != nullptr)
			{
				...
				regen->event = event_create(zodiac_regen_event, info, PASSES_PER_SEC(number(0, 16)) + PASSES_PER_SEC(regen->time));
...
EVENTFUNC(regen_event)
{
	...
	if (info == nullptr)
	{
		sys_err("regen_event> <Factor> nullptr pointer");
		return 0;
	}
	...
	return PASSES_PER_SEC(regen->time + 5); // timer fix
}
