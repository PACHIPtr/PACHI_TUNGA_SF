...
EVENTFUNC(war_begin_event)
{
	...
	if (info == nullptr)
	{
		sys_err("war_begin_event> <Factor> nullptr pointer");
		return 0;
	}
...
EVENTFUNC(war_end_event)
{
	...
	if (info == nullptr)
	{
		sys_err("war_end_event> <Factor> nullptr pointer");
		return 0;
	}
...
EVENTFUNC(war_timeout_event)
{
	...
	if (info == nullptr)
	{
		sys_err("war_timeout_event> <Factor> nullptr pointer");
		return 0;
	}
...
namespace
{
	...
	{
		FBufferedPacket(const void* p, int size) : m_pvData(p), m_iSize(size)
		{
		}

		void operator () (LPCHARACTER ch)
		{
			ch->GetDesc()->BufferedPacket(m_pvData, m_iSize);
		}
...
void CWarMap::BufferedPacket(const void* p, int size)
{
	FBufferedPacket f(p, size);
	std::for_each(m_set_pkChr.begin(), m_set_pkChr.end(), f);
}
...
void CWarMap::UseSkill(BYTE skillVnum, DWORD gIndex)
{
	if (m_TeamData[0].dwID == gIndex)
	{
		if (skillVnum == 151)
			m_TeamData[0].skillone = 1;
		else if (skillVnum == 152)
			m_TeamData[0].skilltwo = 1;
		else if (skillVnum == 153)
			m_TeamData[0].skillthree = 1;
		else if (skillVnum == 154)
			m_TeamData[0].skillfour = 1;
		else if (skillVnum == 155)
			m_TeamData[0].skillfive = 1;
		else if (skillVnum == 156)
			m_TeamData[0].skillsix = 1;
		SendSkillPacket(0);
	}
	else if (m_TeamData[1].dwID == gIndex)
	{
		if (skillVnum == 151)
			m_TeamData[1].skillone = 1;
		else if (skillVnum == 152)
			m_TeamData[1].skilltwo = 1;
		else if (skillVnum == 153)
			m_TeamData[1].skillthree = 1;
		else if (skillVnum == 154)
			m_TeamData[1].skillfour = 1;
		else if (skillVnum == 155)
			m_TeamData[1].skillfive = 1;
		else if (skillVnum == 156)
			m_TeamData[1].skillsix = 1;
		SendSkillPacket(1);
	}
}
...
void CWarMap::SendSkillPacket(BYTE bIdx)
{
	TPacketGCGuild pack;
	TPacketGuildWarSkill pack2;
	pack.header = HEADER_GC_GUILD;
	pack.subheader = GUILD_SUBHEADER_GC_WAR_SKILL;
	pack.size = sizeof(pack) + sizeof(pack2);
	
	pack2.guild_id = m_TeamData[bIdx].dwID;
	pack2.skillone = m_TeamData[bIdx].skillone;
	pack2.skilltwo = m_TeamData[bIdx].skilltwo;
	pack2.skillthree = m_TeamData[bIdx].skillthree;
	pack2.skillfour = m_TeamData[bIdx].skillfour;
	pack2.skillfive = m_TeamData[bIdx].skillfive;
	pack2.skillsix = m_TeamData[bIdx].skillsix;
	
	BufferedPacket(&pack, sizeof(pack));
	Packet(&pack2, sizeof(pack2));
}
...
EVENTFUNC(war_reset_flag_event)
{
	...
	if (info == nullptr)
	{
		sys_err("war_reset_flag_event> <Factor> nullptr pointer");
		return 0;
	}
