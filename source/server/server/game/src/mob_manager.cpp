...
bool CMobManager::Initialize(TMobTable * pTable, int iSize)
{
	...
	for (int i = 0; i < iSize; ++i, ++t)
	{
		...
		//sys_log (0, "MOB: #%-5d %-30s LEVEL %u HP %u DEF %u EXP %u DROP_ITEM_VNUM %u SKILL_COUNT %d", t->dwVnum, t->szLocaleName, t->bLevel, t->dwMaxHP, t->wDef, t->dwExp, t->dwDropItemVnum, SkillCount);
