...
bool CGuild::RemoveMember(DWORD pid)
{
	...
	if (ch)
	{
		...
		if (ch->IsAffectFlag(AFF_KING_GUILD))
			ch->RemoveAffect(AFFECT_KING_GUILD);
#endif
...
void CGuild::LoginMember(LPCHARACTER ch)
{
	...
	{
		if (!ch->IsAffectFlag(AFF_KING_GUILD))
			ch->AddAffect(AFFECT_KING_GUILD, POINT_NONE, 0, AFF_KING_GUILD, INFINITE_AFFECT_DURATION, 0, true);
	}
	else
	{
		if (ch->IsAffectFlag(AFF_KING_GUILD))
...
void CGuild::LoadGuildMemberData(SQLMsg * pmsg)
{
	...
	for (uint i = 0; i < pmsg->Get()->uiNumRows; ++i)
	{
		...
		DWORD pid = strtoul(row[0], (char**)nullptr, 10);
		BYTE grade = (BYTE)strtoul(row[1], (char**)nullptr, 10);
		BYTE is_general = 0;

		if (row[2] && *row[2] == '1')
			is_general = 1;

		DWORD offer = strtoul(row[3], (char**)nullptr, 10);
		BYTE level = (BYTE)strtoul(row[4], (char**)nullptr, 10);
		BYTE job = (BYTE)strtoul(row[5], (char**)nullptr, 10);
		char* name = row[6];
...
void CGuild::LoadGuildGradeData(SQLMsg * pmsg)
{
	...
	*/
	for (uint i = 0; i < pmsg->Get()->uiNumRows; ++i)
	{
		...
		char* name = row[1];
		DWORD auth = strtoul(row[2], nullptr, 10);
...
void CGuild::LoadGuildData(SQLMsg * pmsg)
{
	...
	m_data.level = (BYTE)strtoul(row[1], (char**)nullptr, 10);
	m_data.exp = strtoul(row[2], (char**)nullptr, 10);
	strlcpy(m_data.name, row[3], sizeof(m_data.name));

	m_data.skill_point = (BYTE)strtoul(row[4], (char**)nullptr, 10);
	if (row[5])
...
#endif
#ifdef ENABLE_KING_GUILD_SYSTEM
	str_to_number(m_data.king, row[15]);
#endif
...
{
	if (str.length() > GUILD_COMMENT_MAX_LEN || str.length() == 0) // Added string nullptr verification
		return;
...
void CGuild::DeleteComment(LPCHARACTER ch, DWORD comment_id)
{
	...
	if (GetMember(ch->GetPlayerID())->grade == GUILD_LEADER_GRADE)
		std::unique_ptr<SQLMsg> pmsg(DBManager::instance().DirectQuery("DELETE FROM guild_comment%s WHERE id = %u AND guild_id = %u", get_table_postfix(), comment_id, m_data.guild_id));
	else
		std::unique_ptr<SQLMsg> pmsg(DBManager::instance().DirectQuery("DELETE FROM guild_comment%s WHERE id = %u AND guild_id = %u AND name = '%s';", get_table_postfix(), comment_id, m_data.guild_id, ch->GetName()));
...
void CGuild::RefreshCommentForce(DWORD player_id)
{
	...
	{
		MYSQL_ROW row = mysql_fetch_row(pmsg->Get()->pSQLResult);
		DWORD id = strtoul(row[0], nullptr, 10);
...
EVENTFUNC(GuildInviteEvent)
{
	...
	if (pInfo == nullptr)
	{
		sys_err("GuildInviteEvent> <Factor> nullptr pointer");
		return 0;
	}
