...
EVENTFUNC(party_update_event)
{
	...
	if (info == nullptr)
	{
		sys_err("party_update_event> <Factor> nullptr pointer");
		return 0;
	}
...
void CParty::SendPartyJoinAllToOne(LPCHARACTER ch)
{
	...
	for (TMemberMap::iterator it = m_memberMap.begin();it!= m_memberMap.end(); ++it)
	{
		...
		if (it->second.pCharacter || pCCI)
			UpdateOnlineState(it->first, it->second.strName.c_str(), it->second.mapIdx, it->second.channel);
		else
...
void CParty::SendPartyLinkAllToOne(LPCHARACTER ch)
{
	...
	for (it = m_memberMap.begin(); it != m_memberMap.end(); ++it)
	{
		...
		{
			p.vid = (DWORD)it->second.pCharacter->GetVID();
...
int CParty::ComputePartyBonusExpPercent()
{
	...
	if (leader && (leader->IsEquipUniqueItem(UNIQUE_ITEM_PARTY_BONUS_EXP_PLUS) || leader->IsEquipUniqueItem(UNIQUE_ITEM_PARTY_BONUS_EXP_PLUS_SECOND)))
	{
