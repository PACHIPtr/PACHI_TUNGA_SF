...
namespace quest
{
	...
	EVENTFUNC(quest_server_timer_event)
	{
		...
		if (info == nullptr)
		{
			sys_err("quest_server_timer_event> <Factor> nullptr pointer");
			return 0;
		}
	...
	EVENTFUNC(quest_timer_event)
	{
		...
		if (info == nullptr)
		{
			sys_err("quest_timer_event> <Factor> nullptr pointer");
			return 0;
		}
		...
		END_OF_TIMER_EVENT:
			...
			if (pPC)
				...
				sys_err("quest::PC pointer nullptr. player_id: %u", info->player_id);
