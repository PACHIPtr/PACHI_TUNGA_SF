...
#ifdef ENABLE_ZODIAC_TEMPLE_SYSTEM
	if (GetMapIndex() >= 3580000 && GetMapIndex() < 3590000)
	{
		// sys_err("BURA METIN EFEKTLERINI VERIYOR");
		UpdatePacket();
		return;
	}
#endif
	...
	if (1.0f == fRate)
	{
		...
		else
		{
			if (GetVictim() && !IsCoward())
			{
				...
				{
					MonsterLog("[BATTLE] Entering battle against %s ", GetVictim()->GetName());
				}
			...
			else
			{
				...
				{
					MonsterLog("[IDLE] Entering state. ");
				}
...
void CHARACTER::StateBattle()
{
	...
	if (fDist >= 4000.0f)
	{
		...
		if (bPass)
		{
			MonsterLog(" Too far victim [4000.0f]. Bye ");
			SetVictim(nullptr);
			if (pkChrProtege)
				if (DISTANCE_APPROX(GetX() - pkChrProtege->GetX(), GetY() - pkChrProtege->GetY()) > 1000)
