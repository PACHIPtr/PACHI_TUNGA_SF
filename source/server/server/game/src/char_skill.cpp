...
void CHARACTER::SkillLevelUp(DWORD dwVnum, BYTE bMethod)
{
	...
	if (pkSk->dwType != 0)
	{
		...
		{
		case SKILL_NORMAL:
			// 번섭은 스킬 업그레이드 15~20 사이 랜덤 마스터 수련
			if (GetSkillLevel(pkSk->dwVnum) >= 1)
			{
	...
	void operator () (LPENTITY ent)
	{
		...
		if (!pkChrVictim->Damage(m_pkChr, iDam, dt) && !pkChrVictim->IsStun())
		{
			...
			if (IS_SET(m_pkSk->dwFlag, SKILL_FLAG_REMOVE_GOOD_AFFECT))
			{
				...
				int iDifLev = 9;
				if ((iLevel - iDifLev <= yLevel) || (iLevel >= yLevel))
				{
...
EVENTFUNC(skill_gwihwan_event)
{
	...
	if (info == nullptr)
	{
		sys_err("skill_gwihwan_event> <Factor> nullptr pointer");
		return 0;
	}
...
int CHARACTER::ComputeSkill(DWORD dwVnum, LPCHARACTER pkVictim, BYTE bSkillLevel)
{
	...
	{
		if (test_server)
			sys_log(0, "ComputeSkill: %s Victim == nullptr, skill %d", GetName(), dwVnum);
...
EVENTFUNC(skill_muyoung_event)
{
	...
	if (info == nullptr)
	{
		sys_err("skill_muyoung_event> <Factor> nullptr pointer");
		return 0;
	}
...
EVENTFUNC(mob_skill_hit_event)
{
	...
	if (info == nullptr)
	{
		sys_err("mob_skill_event_info> <Factor> nullptr pointer");
		return 0;
	}
...
#ifdef ENABLE_SKILL_RENEWAL
int CHARACTER::GetSkillBookReadCount(DWORD skillVnum)
{
	quest::CQuestManager & q = quest::CQuestManager::instance();
	quest::PC * pPC = q.GetPC(GetPlayerID());
	char flag[128 + 1];
...
int CHARACTER::GetSkillRemainCount(DWORD skillVnum)
{
	quest::CQuestManager & q = quest::CQuestManager::instance();
	quest::PC * pPC = q.GetPC(GetPlayerID());
	int need_bookcount = GetSkillLevel(skillVnum) - 20;
	char flag[128 + 1];
	memset(flag, 0, sizeof(flag));
	snprintf(flag, sizeof(flag), "traning_master_skill.%u.read_count", skillVnum);
	int read_count = pPC->GetFlag(flag);
	if (read_count > 10)
		return 0;
	if (need_bookcount > 10)
		return 0;

	return (need_bookcount - read_count) + 1;
}
void CHARACTER::SendSkillBookCount()
{
	TPacketGCSkillBookCount p;
	p.header = HEADER_GC_SKILL_BOOK_COUNT;
	
	const DWORD* c_adwAvailSkillVnums = GetUsableSkillList();
	if (!c_adwAvailSkillVnums)
		return;
	
	for (int i = 0; i < 6; ++i)
	{
		p.readCount[i] = GetSkillBookReadCount(c_adwAvailSkillVnums[i]);
		p.leftCount[i] = GetSkillRemainCount(c_adwAvailSkillVnums[i]);
	}
