...
EVENTFUNC(dungeon_dead_event)
{
	...
	if (info == nullptr)
	{
		sys_err("dungeon_dead_event> <Factor> nullptr pointer");
		return 0;
	}
...
struct FCountMonster
{
	...
	void operator()(LPENTITY ent)
	{
		...
		{
			LPCHARACTER ch = (LPCHARACTER)ent;
			if (ch->IsMonster() || ch->IsStone())//fixed should work for monsters and stones, not for players.
				n++;
		}
	}
};
...
namespace
{
	struct FNotice
	{
		...
		void operator() (LPENTITY ent)
		{
			...
			{
				LPCHARACTER ch = (LPCHARACTER)ent;
				if (ch->IsPC()) //fixed. should work for players.
...
EVENTFUNC(dungeon_jump_to_event)
{
	...
	if (info == nullptr)
	{
		sys_err("dungeon_jump_to_event> <Factor> nullptr pointer");
		return 0;
	}
...
EVENTFUNC(dungeon_exit_all_event)
{
	...
	if (info == nullptr)
	{
		sys_err("dungeon_exit_all_event> <Factor> nullptr pointer");
		return 0;
	}
