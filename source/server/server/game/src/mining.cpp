...
namespace mining
{
	...
	void OreDrop(LPCHARACTER ch, DWORD dwLoadVnum)
	{
		...
		if (quest::CQuestManager::instance().GetEventFlag("mining_event") == 1)
		{
			ch->SetQuestFlag("mining_event.currentMiningPoint", ch->GetQuestFlag("mining_event.currentMiningPoint") + 1);
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%dREWARD_MINING_POINTS"), 1);
			LogManager::instance().MiningEventLog(ch->GetPlayerID(), ch->GetName(), ch->GetQuestFlag("mining_event.currentMiningPoint"));
		}
	...
	{
		return (number(0, pick.GetValue(1)) == 1);
	}
