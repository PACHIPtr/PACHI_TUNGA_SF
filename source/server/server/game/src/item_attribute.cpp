...
int CItem::GetRarePoints()
{
	return m_aAttr[7].sValue;
}

int CItem::GetRareLevel()
{
	int iCurAmount = GetRarePoints();
	
	if (iCurAmount >= 10 && iCurAmount <= 25)
		return 1;
	else if (iCurAmount > 25 && iCurAmount <= 50)
		return 2;
	else if (iCurAmount > 50 && iCurAmount <= 75)
		return 3;
	else if (iCurAmount > 75)
		return 4;
	
	return 0;
}

void CItem::UpdateRarePoints(int i)
{
	int iOldRareLevel = GetRareLevel();
	m_aAttr[7].sValue = m_aAttr[7].sValue + i;
	UpdatePacket();
	Save();
	if (GetRareLevel() > iOldRareLevel && m_pOwner)
		m_pOwner->ViewReencode();
}

void CItem::NegativeRarePoints(int i)
{
	int iOldRareLevel = GetRareLevel();
	m_aAttr[7].sValue = m_aAttr[7].sValue - i;
	UpdatePacket();
