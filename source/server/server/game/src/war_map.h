...
public:
	...
	void	BufferedPacket(const void* pv, int size);
	void	Notice(const char* psz);
	void	SendWarPacket(LPDESC d);
	void	SendScorePacket(BYTE bIdx, LPDESC d = nullptr);
	void	SendSkillPacket(BYTE bIdx);
	...
	void	UseSkill(BYTE skillVnum, DWORD gIndex);
private:
