...
bool P2P_MANAGER::Send(const void* c_pvData, int iSize, WORD wPort, bool bP2PPort)
{
	TR1_NS::unordered_set<LPDESC>::iterator it = m_set_pkPeers.begin();
	...
	while (it != m_set_pkPeers.end())
	{
		...
		if ((bP2PPort && pkDesc->GetP2PPort() == wPort)) {
			sys_log(0, "P2P_Send: send to port %u", wPort);
			pkDesc->Packet(c_pvData, iSize);
			return true;
		}
		else
			sys_log(0, "P2P_Send: wrong port %u [p2p %u] [searched %u]", pkDesc->GetPort(), pkDesc->GetP2PPort(), wPort);
	}
...
void P2P_MANAGER::Login(LPDESC d, const TPacketGGLogin * p)
{
	...
	CPartyManager::instance().P2PLogin(pkCCI->dwPID, pkCCI->szName, pkCCI->lMapIndex, pkCCI->bChannel);
#else
