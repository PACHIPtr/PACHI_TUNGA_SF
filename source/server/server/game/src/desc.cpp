...
EVENTFUNC(ping_event)
{
	if (event == nullptr) 
	{
		sys_err("%s <Event> nullptr pointer, %s:%d", __FUNCTION__, __FILE__, __LINE__);
		return 0;
	}
	...
	if (info == nullptr)
	{
		sys_err("ping_event> <Factor> nullptr pointer");
		return 0;
	}
...
#ifndef _IMPROVED_PACKET_ENCRYPTION_
	thecore_memcpy(m_adwEncryptionKey, "PraE5Jg43fZsG6pt", sizeof(DWORD) * 4);
	thecore_memcpy(m_adwDecryptionKey, "PraE5Jg43fZsG6pt", sizeof(DWORD) * 4);
...
void DESC::Log(const char* format, ...)
{
	...
	fprintf(m_pLogFile, "%02d %02d %02d:%02d:%02d | ", tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
...
EVENTFUNC(disconnect_event)
{
	...
	if (info == nullptr)
	{
		sys_err("disconnect_event> <Factor> nullptr pointer");
		return 0;
	}
