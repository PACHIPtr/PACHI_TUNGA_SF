#include "stdafx.h"
#include <sstream>
#include "../../libgame/libgame/targa.h"
#include "../../libgame/libgame/attribute.h"
#include "config.h"
#include "utils.h"
...
bool SECTREE_MANAGER::LoadAttribute(LPSECTREE_MAP pkMapSectree, const char* c_pszFileName, TMapSetting & r_setting)
{
	...
	for (int y = 0; y < iHeight; ++y)
	{
		for (int x = 0; x < iWidth; ++x)
		{
			...
			if (tree == nullptr)
			{
				sys_err("FATAL ERROR! LoadAttribute(%s) - cannot find sectree(package=%x, coord=(%u, %u), map_index=%u, map_base=(%u, %u))", c_pszFileName, id.package, id.coord.x, id.coord.y, r_setting.iIndex, r_setting.iBaseX, r_setting.iBaseY);
			...
			{
				sys_err("returned tree id mismatch! return %u, request %u", tree->m_id.package, id.package);
				fclose(fp);
			...
			{
				sys_err("SECTREE_MANAGER::LoadAttribte : %s : %d %d size mismatch! %d", c_pszFileName, tree->m_id.coord.x, tree->m_id.coord.y, uiDestSize);
				fclose(fp);
...
struct FPurgeMonsters
{
	void operator() (LPENTITY ent)
	{
		if (ent->IsType(ENTITY_CHARACTER) == true)
		{
			...
			if (lpChar->IsMonster() == true && !lpChar->IsPet() && !lpChar->IsNewPet() && !lpChar->IsSupportShaman() && !lpChar->IsMount())
#else
			if (lpChar->IsMonster() == true && !lpChar->IsPet() && !lpChar->IsSupportShaman() && !lpChar->IsMount())
#endif
			{
...
#ifdef ENABLE_GROWTH_PET_SYSTEM
			if (lpChar->IsNPC() == true && !lpChar->IsPet() && !lpChar->IsNewPet() && !lpChar->IsSupportShaman() && !lpChar->IsMount())
#else
			if (lpChar->IsNPC() == true && !lpChar->IsPet() && !lpChar->IsSupportShaman() && !lpChar->IsMount())
#endif
			{
				M2_DESTROY_CHARACTER(lpChar);
			}
