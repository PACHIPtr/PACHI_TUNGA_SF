...
#include <queue>
#include <utility>
#include "../../libsql/libsql/libsql.h"
...
#else
	void	StartWar(BYTE bType, DWORD GID1, DWORD GID2, CGuildWarReserve* pkReserve = nullptr);
#endif
...
private:
	...
	std::priority_queue<stPairGuildWar, std::vector<stPairGuildWar>, std::greater<stPairGuildWar> > m_pqOnWar;
	std::priority_queue<stPairWaitStart, std::vector<stPairWaitStart>, std::greater<stPairWaitStart> > m_pqWaitStart;
	std::priority_queue<stPairSkillUsed, std::vector<stPairSkillUsed>, std::greater<stPairSkillUsed> > m_pqSkill;
