...
void CClientManager::QUERY_PARTY_SET_MEMBER_MAP_INDEX(CPeer* peer, TPacketPartySetMemberMapIndex* p)
{
	TPartyMap& pm = m_map_pkChannelParty[peer->GetChannel()];
	auto it = pm.find(p->dwLeaderPID);
	...
	if (it == pm.end())
	{
		sys_err("PARTY SetMemberMapIndex - Non exists [%lu] cannot level change [%lu]", p->dwLeaderPID, p->dwPID);
		return;
	}
	...
	if (pit == it->second.end())
	{
		sys_err("PARTY SetMemberMapIndex - Cannot find [%lu] in party [%lu]", p->dwPID, p->dwLeaderPID);
		return;
	}

	pit->second.mapIdx = p->mapIdx;

	ForwardPacket(HEADER_DG_PARTY_SET_MEMBER_MAP_INDEX, p, sizeof(TPacketPartySetMemberMapIndex), peer->GetChannel());
	sys_log(0, "PARTY SetMemberMapIndex pid [%lu] mapIdx %d", p->dwPID, p->mapIdx);
}

void CClientManager::QUERY_PARTY_SET_MEMBER_CHANNEL(CPeer* peer, TPacketPartySetMemberChannel* p)
{
	TPartyMap& pm = m_map_pkChannelParty[peer->GetChannel()];
	auto it = pm.find(p->dwLeaderPID);
	...
	if (it == pm.end())
	{
		sys_err("PARTY SetMemberChannel - Non exists [%lu] cannot level change [%lu]", p->dwLeaderPID, p->dwPID);
		return;
	}
	...
	if (pit == it->second.end())
	{
		sys_err("PARTY SetMemberChannel - Cannot find [%lu] in party [%lu]", p->dwPID, p->dwLeaderPID);
		return;
	}

	pit->second.channel = p->channel;

	ForwardPacket(HEADER_DG_PARTY_SET_MEMBER_CHANNEL, p, sizeof(TPacketPartySetMemberChannel), peer->GetChannel());
	sys_log(0, "PARTY SetMemberChannel pid [%lu] level %d", p->dwPID, p->channel);
}
#endif