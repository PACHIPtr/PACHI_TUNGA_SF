...
void CGuildManager::ParseResult(SQLResult * pRes)
{
	...
	while ((row = mysql_fetch_row(pRes->pSQLResult)))
	{
		...
		str_to_number(r_info.king, row[8]);
#endif
	}
}
...
void CGuildManager::Initialize()
{
	char szQuery[1024];
#ifdef ENABLE_KING_GUILD_SYSTEM
	snprintf(szQuery, sizeof(szQuery), "SELECT id, name, ladder_point, win, draw, loss, gold, level, king FROM guild%s", GetTablePostfix());
#else
...
#ifdef ENABLE_KING_GUILD_SYSTEM
	snprintf(szQuery, sizeof(szQuery), "SELECT id, name, ladder_point, win, draw, loss, gold, level, king FROM guild%s WHERE id=%u", GetTablePostfix(), dwGuildID);
#else
...
#ifdef ENABLE_IMPROVED_GUILD_WAR_SYSTEM
		StartWar(ws.bType, ws.GID[0], ws.GID[1], ws.pkReserve, ws.iMaxPlayer, ws.iMaxScore); // insert new element to m_WarMap and m_pqOnWar
#else
...
{
#ifdef ENABLE_IMPROVED_GUILD_WAR_SYSTEM
	decltype(m_DeclareMap.begin()) it = m_DeclareMap.find(TGuildDeclareInfo(0, guild_from, guild_to, iMaxPlayer, iMaxScore));
#else
...
#ifdef ENABLE_IMPROVED_GUILD_WAR_SYSTEM
	TGuildWaitStartInfo info(p->bType, p->dwGuildFrom, p->dwGuildTo, p->lWarPrice, p->lInitialScore, nullptr, p->iMaxPlayer, p->iMaxScore);
#else
