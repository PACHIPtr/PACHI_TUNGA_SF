...
void CClientManager::RESULT_LOGIN_BY_KEY(CPeer * peer, SQLMsg * msg)
{
	...
	extern std::string g_stLocale;
	snprintf(szQuery, sizeof(szQuery), "SELECT id, name, job, level, playtime, st, ht, dx, iq, part_main, part_hair, part_sash, x, y, skill_group, change_name, UNIX_TIMESTAMP(last_play) FROM player%s WHERE account_id=%u", GetTablePostfix(), info->pAccountTable->id);
		...
		for (j = 0; j < PLAYER_PER_ACCOUNT; ++j)
		{
			if (pkTab->players[j].dwID == player_id)
			{
				...
				if (pt)
				{
					...
					str_to_number(pkTab->players[j].last_play, row[16]);
				}
				else
				{
					if (!row[col++])
...
void CClientManager::RESULT_LOGIN(CPeer * peer, SQLMsg * msg)
{
	...
	if (info->account_index == 0)
	{
		...
		else
		{
			...
			char queryStr[512];
			snprintf(queryStr, sizeof(queryStr), "SELECT id, name, job, level, playtime, st, ht, dx, iq, part_main, part_hair, part_sash, x, y, skill_group, change_name, UNIX_TIMESTAMP(last_play) FROM player%s WHERE account_id=%u", GetTablePostfix(), info->pAccountTable->id);
