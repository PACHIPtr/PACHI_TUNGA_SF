...
quest pvp_manager begin
	state start begin
		...
		function guard_reset()
			notice_in_map(""..pc.getname().." Arenadan uzaklaþtýrýldý!")
			pc.warp(6598400,6598400)
		end
		
		
		
		when 11001.chat."PvP Arenasýna Git " or 11003.chat."PvP Arenasýna Git " or 11005.chat."PvP Arenasýna Git " begin
			say_title("Savaþ Sorumlusu: ")
			say("Hey Evet Sen.")
			say("Sen gerçekten çok güçü görünüyorsun.")
			say("Turnuva'da senin gibi oyuncularla savaþacaksýn.")
			say("PvP Arenasý'nda Tunuva Sorumlusu'nu bul ve ")
			say("Turnuva'ya kayýt ol.")
			if game.get_event_flag("pvp") == 0 then
				wait()
				say_title("Savaþ Sorumlusu: ")
				say("Turnuvaya katýlmana izin veriyorum. ")
				...
				wait()
				say_title("Savaþ Sorumlusu: ")
				say("Turnuvaya katýlmak istiyormusun?")
		...
		when 20078.chat."Test " begin
			say(" "..pvp_manager.get_duel_partner().." ")
		end	
		...
		function open_registrations()
			...
			notice_all("PvP Turnuvasý baþlamýþtýr.Savaþ Sorumulusu'ndan giriþ yapabilirsiniz.")
		...
		-- not working
		--[[
		when 20351.chat."GM: Düelloyu bitir " with pc.is_gm() and game.get_event_flag("pvp") == 2 begin
		...
		function fight_init(pid1, pid2, arena_id)
			...
			if pvp_manager.is_player_in_map(tostring(pvp_attenders[pid1].name)) == false then
				notice_in_map(""..tostring(pvp_attenders[pid1].name).." adlý oyuncu oyunda olmadýðýndan dolayý diskalifiye edildi.", pc.get_map_index())
			...
			if pvp_manager.is_player_in_map(tostring(pvp_attenders[pid2].name)) == false then
				notice_in_map(""..tostring(pvp_attenders[pid2].name).." adlý oyuncu oyunda olmadýðýndan dolayý diskalifiye edildi.", pc.get_map_index())
