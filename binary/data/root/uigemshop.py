...
class GemShopWindow(ui.ScriptWindow):
	...
	def OverInItem(self, slotIndex):
		...
		if 0 != self.tooltipItem:
			...
			if int(self.pageNumber == 1):
				self.tooltipItem.SetItemToolTip(player.GetGemShopItemVnum(slotIndex, False))
			elif int(self.pageNumber == 2):
				self.tooltipItem.SetItemToolTip(player.GetGemShopItemVnum(slotIndex+9, False))
			elif int(self.pageNumber == 3):
				self.tooltipItem.SetItemToolTip(player.GetGemShopItemVnum(slotIndex+18, False))
	...
	def UseItemSlot(self, slotIndex):
		...
		if int(self.pageNumber == 1):
			if player.GetItemCountByVnum(50926) < player.GetGemShopItemPrice(slotIndex, False):
				chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.GEM_SYSTEM_NOT_ENOUGH_HP_GEM)
			else:
				self.GemShopSlotBuy(slotIndex)
		elif int(self.pageNumber == 2):
			if player.GetItemCountByVnum(50926) < player.GetGemShopItemPrice(slotIndex+9, False):
				chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.GEM_SYSTEM_NOT_ENOUGH_HP_GEM)
			else:
				self.GemShopSlotBuy(slotIndex+9)
		elif int(self.pageNumber == 3):
			if player.GetItemCountByVnum(50926) < player.GetGemShopItemPrice(slotIndex+18, False):
				chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.GEM_SYSTEM_NOT_ENOUGH_HP_GEM)
			else:
				self.GemShopSlotBuy(slotIndex+18)
	...
	def RefreshItemSlot(self):
		for i in xrange(self.GEM_SHOP_SLOT_MAX):
			if int(self.pageNumber == 1):
				itemCount = player.GetGemShopItemCount(i, False)
				itemPrice = player.GetGemShopItemPrice(i, False)
			elif int(self.pageNumber == 2):
				itemCount = player.GetGemShopItemCount(i+9, False)
				itemPrice = player.GetGemShopItemPrice(i+9, False)
			elif int(self.pageNumber == 3):
				itemCount = player.GetGemShopItemCount(i+18, False)
				itemPrice = player.GetGemShopItemPrice(i+18, False)

			if itemCount <= 1:
				itemCount = 0
			
			if int(self.pageNumber == 1):
				self.itemSlot.SetItemSlot(i, player.GetGemShopItemVnum(i, False), itemCount)
			elif int(self.pageNumber == 2):
				self.itemSlot.SetItemSlot(i, player.GetGemShopItemVnum(i+9, False), itemCount)
			elif int(self.pageNumber == 3):
				self.itemSlot.SetItemSlot(i, player.GetGemShopItemVnum(i+18, False), itemCount)
