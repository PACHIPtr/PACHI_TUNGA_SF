...
class SelectItemWindow(ui.ScriptWindow):
	...
	def Close(self):
		...
		net.SendSelectItemPacket(player.INVENTORY, 6969)
		self.OverOutItem()
		self.Hide()
	...
	def SelectItemSlot(self, slotPos):
		...
		net.SendSelectItemPacket(player.STONE_ITEMS_INVENTORY, inventorySlotPos)
		self.RefreshSlot()
		self.OverOutItem()
	...
	def RefreshSlot(self):
		...
		for i in xrange(player.INVENTORY_PAGE_SIZE*player.INVENTORY_PAGE_COUNT):
			...
			itemVNum = getItemVNum(player.STONE_ITEMS_INVENTORY, slotNumber)
			if 0 == itemVNum:
				continue
			...
			itemGrade = player.GetItemGrade(player.STONE_ITEMS_INVENTORY, slotNumber)
			if itemGrade > 3:
				continue
		...
		for selectWndPos, inventoryPos in self.inventorySlotPosDict.items():
			itemVNum = getItemVNum(player.STONE_ITEMS_INVENTORY, inventoryPos)
			itemCount = getItemCount(player.STONE_ITEMS_INVENTORY, inventoryPos)
	...
	def OverInItem(self, slotIndex):
		if None != self.tooltipItem:
			...
			self.tooltipItem.SetInventoryItem(inventorySlotPos, player.STONE_ITEMS_INVENTORY)
